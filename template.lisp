...
(defmacro deflayout (name (data-var &key (stream-var 's) top-level) &body body)
  ...
  (let ((view-name (intern (string-upcase (format nil "layout-~a" name)) :papaya)))
    `(progn
        ...
        (let ((colon-pos (position #\: line)))
          (when colon-pos
            (let* ((key (intern (string-upcase (subseq line 0 colon-pos)) :keyword))
              ...
              (case key
                (:layout (setf value (intern (string-upcase value) :papaya))))
...
(defun layout (name data)
  ...
  (let ((layout-fn (intern (string-upcase (format nil "layout-~a" name)) :papaya)))
